...
import {getOrganazition,organazitionModify,newDepartment,deleteDepartment} from "../../services/BookAddress/BookAddress"
...
const loop = ({data, draggable, isCheck, changeReviseState,addChildDep,deleteDep,singleOrMultiple,selectedData,reviseDepName}) => data.map((item) => {
    // console.log(data);
        ...
        <div className={styles.reviseContainer}>
            ...
            <div className={styles.reviseEdite}>
                ...
                <Button className={styles.reviseBtn + " " + styles.reviseConfirm} onClick={() => {
                    ...
                    reviseDepName(item["inputValue"],item["key"],{callBack:changeReviseState,params:{
                        key:item["key"],
    ...
    let confirmModal = (obj) => {
        let tempObj = {
            ...
            onOk() {
                if(obj["type"] === "add"){
                    let newId = Guid();
                    newDepartment({ParentId:item["key"],Id:newId,Name:newDepName,Platform:Permission}).then(res=>{
                        console.log(res);
                        if(res.data.isValid){
                ...
                if(obj["type"] === "delete"){
                    ...
                    deleteDepartment({EntityIdList:tempArr}).then(res=>{
                        console.log(res);
                        if(res.data.isValid){
        ...
        <Menu style={{ padding: 0 }} onClick={(e) => {
            ...
            switch (key) {
                ...
                case "1":
                    ...
                    confirmModal({ title: "新建部门", content: <Input autoFocus onChange={(e)=>{
                        newDepName = e.target.value;
                    }}/>, icon: "none",type:"add" });
                    break;
                case "2":
                    changeReviseState(item["key"], { isDownVisible: false });
                    confirmModal({ title: `您确定要删除“${item["title"]}”吗？`, content: "删除部门会将部门所关联的权限一并删除，此操作不可逆", icon: "",type:"delete" });
                    break;
            }
        }}>
    ...
    let judgeChecked = singleOrMultiple === 0 ? (selectedData.length?selectedData[0]["id"] === item["key"]:false):item["checked"];
    let customItem = (
        <span id={item["key"]} style={{ position: isCheck?"relative":"static", display: "inline-block", paddingRight:"20px",width: "calc(100% - 24px)" }}>{item.title}
            {isCheck && <div style={{borderRadius:singleOrMultiple===0?"50%":0}} className={`${styles.customCheck} ${judgeChecked ? styles.customChecked : ""}`}>
                {
                    judgeChecked && <Icon type="check" theme="outlined" className={styles.checkIcon} />
                }
            </div>}
            {draggable && item["reviseNameState"] === 1 && <Dropdown getPopupContainer={() => document.getElementById(item["key"])} visible={item["isDownVisible"]} overlay={menu} trigger={['click']} onVisibleChange={(visible) => {
                // console.log(visible); 
                changeReviseState(item["key"], { isDownVisible: visible });
            }}>
                <Icon className={styles.creditIcon} type="ellipsis" theme="outlined" />
            </Dropdown>}
            {draggable && item["reviseNameState"] === 2 && <Dropdown getPopupContainer={() => document.getElementById(item["key"])} visible={item["isDownVisible"]} overlay={ReviseDropDown} trigger={['click']} onVisibleChange={(visible) => {
    ...
    if (item.children && item.children.length) {
        return <TreeNode type={item["type"]} icon={<Icon className={styles.treeIcon} type={item.type=="0"?"fork":"user"} theme="outlined" />} isLeaf={!item.isLeaf} key={item.key} title={customItem}>{loop({data:item.children, draggable, isCheck, changeReviseState,addChildDep,deleteDep,singleOrMultiple,selectedData,reviseDepName})}</TreeNode>;
    }
    return <TreeNode type={item["type"]} icon={<Icon className={styles.treeIcon} type={item.type=="0"?"fork":"user"} theme="outlined" />} isLeaf={!item.isLeaf} key={item.key} title={customItem} />;
});
/* const onDrop = (info, treeData, changeTreeData) => {
    console.log(info);
    const dropKey = info.node.props.eventKey;
    const dragKey = info.dragNode.props.eventKey;
    const dropPos = info.node.props.pos.split('-');
    const dropPosition = info.dropPosition - Number(dropPos[dropPos.length - 1]);//这个值应该是 放入的位置是在前还是后
    let loop = (data, key, callback) => {
        data.forEach((v, index, arr) => {
            if (v["key"] === key) {
                callback(v, index, arr);
            }
            if (v["children"] && v["children"].length) {
                loop(v["children"], key, callback);
            }
        });
    }
    let tempObj; //拖动的项
    ...
    if (info.dropToGap) { //dropToGap指的是 是在 拖动 目标的 缝隙里还是在他的 上面
        let i, ar;
        loop(treeData, dropKey, (v, index, arr) => {
            i = index;
            ar = arr;
        });
        if (dropPosition === -1) {
...
class TreeCom extends Component {
    ...
    /* componentWillReceiveProps(nextProps){
        if(!is(fromJS(nextProps.treeData),fromJS(this.state.treeData))){
            this.setState({
                treeData:changeData(formateData(nextProps.treeData))
            })
        }
    } */
    // 这个方法有个bug，就是在 没有新的 props 传进来的时候依然 更新了
    /* static getDerivedStateFromProps(nextProps, prevState) {
        console.log(nextProps);
        if(!is(fromJS(nextProps.treeData),fromJS(prevState.treeData))){
            console.log("执行");
            return {
                treeData:changeData(formateData(nextProps.treeData))
            }
    ...
    addChildDep({parentId,childObj}){
        ...
        if(!isLoaded.length){
            ...
            tempArr = reviseTreeData({treeData,key:parentId,callback:({item})=>{
                !item["isLeaf"] && (item["isLeaf"] = true);
            }})
        }else{
            tempArr = reviseTreeData({treeData,key:parentId,childObj,callback:({item,childObj})=>{
    ...
    deleteDep({key}){
        ...
        tempArr = [];
        tempArr = reviseTreeData({treeData,key,callback:({index,treeData,parentItem})=>{
    ...
    render() { 
        let { selectedKeys, onSelect, /* draggable, */ isCheck,multiple,styleList,singleOrMultiple,selectedData,reviseDepName } = this.props;
        ...
        return (
            <Tree
                ...
                onDrop={(info) => { this.onDrop(info, treeData) }}
                // defaultExpandedKeys={[treeData[0]["key"]]}
                loadData = {this.loadData.bind(this)}
                loadedKeys = {loadedKeys}
                selectedKeys={selectedKeys?[...selectedKeys]:[]} /* 这种写法是必须的，感觉内部 当 selectedKeys 的数据引用问题 */
                onSelect={(key, e) => { onSelect && onSelect(key[0], e, undefined,treeData); }}
            >
                {loop({data:treeData, draggable, isCheck, changeReviseState,addChildDep,deleteDep,singleOrMultiple,selectedData,reviseDepName})}
            </Tree>
        );
    }
}
