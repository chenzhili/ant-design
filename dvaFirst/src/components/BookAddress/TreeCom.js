...
const loop = ({ data, draggable, isCheck, changeReviseState, addChildDep, deleteDep, singleOrMultiple, selectedData, reviseDepName }) => data.map((item) => {
        ...
        let tempObj = {
            ...
            onOk() {
                if (obj.type === "add") {
                    ...
                    debugger
                    newDepartment({ ParentId: item.key, Id: newId,DepartmentId:item.departmentId, Name: newDepName, Platform: Permission }).then(res => {
                        console.log(res);
                        if (res.data.isValid) {
    ...
    let judgeChecked = singleOrMultiple === 0 ? (selectedData.length ? selectedData.some(i=>i.id === item.key) : false) : item.checked;
    let customItem = (
    ...
    if (item.children && item.children.length) {
        return <TreeNode type={item.type} dep={item.departmentId} icon={<Icon className={styles.treeIcon} type={item.type == "0" ? "fork" : "user"} theme="outlined" />} isLeaf={!item.isLeaf} key={item.key} title={customItem}>{loop({ data: item.children, draggable, isCheck, changeReviseState, addChildDep, deleteDep, singleOrMultiple, selectedData, reviseDepName })}</TreeNode>;
    }
    return <TreeNode type={item.type} dep={item.departmentId} icon={<Icon className={styles.treeIcon} type={item.type == "0" ? "fork" : "user"} theme="outlined" />} isLeaf={!item.isLeaf} key={item.key} title={customItem} />;
});
