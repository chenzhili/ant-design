...
class FilterPart extends Component {
    constructor(props) {
        super(props);
        let { controlFilterPart: { showFilterArr, selectedData } } = this.props;
        this.state = {
            ...
            defList: [{ name: "admin", id: "475FC3FA-9833-416D-A1C5-E1E885E24C23" }],//默认参数,不管是 部门还是 成员
        }
        // this._searchData({});
    }
    componentWillMount() {
    ...
    async _getMemList(params) {
        ...
        if (data && data.employeeList instanceof Array) {
            let tempArr = this.state.selectedData.length ? initCheckboxState(data.employeeList, this.state.selectedData) : data.employeeList;
            this.setState({
                memArr: tempArr
            });
        }
    ...
    selectRolesOrMem(key, type, bool) {
        ...
        let { rolesArr, selectedData, memArr, materDataArr, defList } = this.state, { singleOrMultiple } = this.props.controlFilterPart;
        let tempArr = [], name = "";
        switch (type) {
            case -1:
        ...
        let multipleModel = (selectedData, key, bool) => {
            console.log(selectedData, key, bool);
            ...
            if (bool) {
                selectedData.push({ ...existItem, type })
            } else {
        ...
        let { selectedData: tempSelectedData } = singleOrMultiple === 0 ? singleModle(selectedData, key, bool) : multipleModel(selectedData, key, bool);
    ...
    render() {
        ...
        let { tabState, isSearch, selectedData, departArr, rolesArr, memArr, searchDataArr, materDataArr, defList, selectTreeKeys, selectRolesActive } = this.state;
