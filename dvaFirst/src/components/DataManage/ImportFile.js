...
function ImportFile(props) {
    let { importSteps, changeSteps,isShowImportModal,operateImportModal } = props;
    return (
        <Modal
            title="导入Excel数据"
        ...
        >
            ...
            {importSteps === 4 && <div className={styles.stepsFinish}>
                <div className={styles.finishContainer}>
                    ...
                    <p>您成功导入了0条数据到表单[出差申请]中</p>
                </div>
            </div>}
            {
                importSteps !== 1 && <div className={styles.procedureBtn}>
                    {importSteps !== 4 && <div className={styles.next} onClick={() => { changeSteps(importSteps + 1); }}>下一步</div>}
                    {importSteps !== 4 && <div className={styles.prev} onClick={() => { changeSteps(importSteps - 1); }}>上一步</div>}
                    { importSteps === 4 && <div className={styles.next} onClick={() => {operateImportModal(false);changeSteps(1);}}>完成</div>}
                </div>
            }
        </Modal>
    );
