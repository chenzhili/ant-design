...
function DelBody(props) {
    ...
    return (
        <td {...other} onClick={(e) => { e.stopPropagation(); }} style={{ textAlign: "center" }}>
            {
...
function DelHeader(props) {
    ...
    return (
        <th {...other}>
            {
                <Checkbox indeterminate={allChecked === 1} checked={allChecked === 2} onChange={onChange.bind(null, props.id)} />
            }
        </th>
    );
}
...
const _getActualyWidth = (columns, tableWidth) => {
    ...
    let total = dataColumns.reduce((prev, next) => {
        ...
        return prev + ((typeof next.width === "string") ? parseInt(next.width) : next.width) * (next.colSpan ? next.colSpan : 1);
    }, 0);
    //console.log(total);
    return total;
}
    ...
    columns.forEach((item) => {
        if (item.isData) {
            if (item.id === "del") {
                ...
                item.onHeaderCell = column => ({
                    width: column.width,
                    isFirst: !item.parentId && item.id !== "del" && item.id !== "coop",
...
const _addDeleteColumn = (columns, showDelete, x) => {
    ...
    if (x === "100%") {
        const generalItems = columns.filter(item => (item.fixed !== "left" && item.fixed !== "right"));
        if (generalItems.length) {
            let i = columns.indexOf(generalItems[generalItems.length - 1]);
...
class TableCom extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if (nextProps.columns.length !== prevState.columns.length || !is(nextProps.columns, prevState.columns)) {
            ...
            let percent = ActualyWidth ? (ActualyWidth / prevState.tableWidth * 100) : 99;
    ...
    changeColumnFixed(id, e) {
        //console.log(id, e);
        let { columns, x } = this.state, type = e.target.value;
    ...
    changeCheckStatus(id, checked) {
        ...
        if (id === "del") {
            tableData.forEach(item => { item.checked = checked });
        } else {
    ...
    initOnRowConfig(record) {
        ...
        let tempObj = {};
        tempObj.onClick = rowClick ? () => { resizeableCustom ? rowClick(record.mainId) : rowClick(record.id) } : null;
        /* 这个 hover 可以先不要 */
        /* if (resizeableCustom) {
    ...
    render() {
        ...
        // 可以 定制 header的宽度
        resizeableCustom && (columns = _addHeaderFun({ columns, handleWidthResize: this.handleWidthResize, changeSort: this.changeSort, changeCheckStatus: this.changeCheckStatus, tableData, changeColumnFixed: this.changeColumnFixed }));
        return (
